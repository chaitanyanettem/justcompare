...
#order by rank desc;	
		...
		what_prices    = "hs18_price,fk_price,infibeam_price,indiaplaza_price,crossword_price,bookadda_price,landmark_price,"
		...
		what_total     = what_essential+what_prices+what_images+what_links+what_misc
		whereq 		   = "fts @@ to_tsquery('english','{0}')".format(query) 
		#myvar = dict('lang':'english','query':'input_q','normalization':1)
		ordered = db.select('books_bookdata', what=what_total,where=whereq,order="rank DESC", limit=20)
