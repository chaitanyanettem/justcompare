		...
		try:
			...
			while True:
				try:
					...
					logging = str(datetime.now())+": Connection problem. Sleeping for 10 seconds.\n"+traceback.print_exc()
					log.write(logging)
					time.sleep(10)
			count += 1
...
page = requests.get("http://www.flipkart.com/books")
...
try:
	while True:
		try:
			...
			con =psycopg2.connect(database='compare', user='shivkanchan', password='shivkanchan')
			cur=con.cursor()
			i1=soup4.find('table',{'class':'more-detail-tb'})
			if (pattern2.findall(i1.text)):
				i2=(pattern2.findall(i1.text))[0]
			elif (pattern3.findall(i1.text)):
			...
			if d7.p is None or d7.p.span is None:
				...
				con.commit()
				continue
			cur.execute("select * from description where isbn=%(isbn)s",{'isbn':isbn13})
			row=cur.fetchone()
			if row is None:
				cur.execute("insert into description values(%s,%s)",(isbn13,d7.p.span.text[:999]))
			else:
				cur.execute("update description set des=%s where isbn=%s",(d7.p.span.text[:999],isbn13))
			con.commit()
		except Exception, e:
			traceback.print_exc()
