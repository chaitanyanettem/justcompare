...
def requests_passable(url):
    ...
    while count < max_repeats:
        try:
            ...
            traceback.print_exc()
            logging = ''.join(
                [str(datetime.now()), ":", traceback.format_exc()])
            ...
            while True:
                try:
                    ...
                    logging = ''.join([str(datetime.now()), ": Connection problem.\
...
def update_categories():
    ...
    for link in all_category_links:
        category_link = ''.join(
            ['http://www.flipkart.com', link.find('a').get('href')])
...
def database_insert():
    for isbn in data:
        cur.execute(
            "select * from books_bookdata where isbn=%(isbn)s", {"isbn": isbn})
        row = cur.fetchone()
        if row is None:
            cur.execute("insert into books_bookdata(isbn,book_name,author_name,fk_price,fk_booklink) values(%s,%s,%s,%s,%s)", (
                isbn, data[isbn][0], data[isbn][1], data[isbn][2], data[isbn][3]))
        else:
            if (row[2] is None and not(data[isbn][1] is '[?]')):
                cur.execute(
                    "update books_bookdata set author_name=%s where isbn=%s", (data[isbn][1], isbn))
            cur.execute("update books_bookdata set fk_price=%s, fk_booklink=%s where isbn=%s", (
                data[isbn][2], data[isbn][3], isbn))
        con.commit()
...
while True:
    ...
    url = ''.join(
        [current_category[:pr_location + 3], url_parameters[0], url_parameters[1], url_parameters[2]])
    while True:
        current_category_page = requests_passable(url.format(number=number))
        number += 20
        soup = BeautifulSoup(BeautifulSoup(str(current_category_page.text)).prettify(
            formatter=None))
        # no-results check:
        ...
        for book in book_boxes:
            ...
            book_link = ''.join(
                ["http://www.flipkart.com", a_links[0].get('href')])
            ...
            #[3:] removes leading 'Rs. '
            book_price = int((book_price.strip())[3:])
